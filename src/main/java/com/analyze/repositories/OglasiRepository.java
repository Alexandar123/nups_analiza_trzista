...
public interface OglasiRepository extends CrudRepository<AdvertiseWebNekretnine, Long>{
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE  UPPER(full_address) LIKE %:address% AND type_of_property = :type_of_property AND type_of_ad = :type_of_ad AND areas BETWEEN :areasmin AND :areasmax AND LOWER(city) LIKE :city AND UPPER(state) LIKE :state AND ad_published BETWEEN :from AND :to", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE type_of_property = ?1 AND type_of_ad = ?2 ORDER BY ad_published", nativeQuery = true)
