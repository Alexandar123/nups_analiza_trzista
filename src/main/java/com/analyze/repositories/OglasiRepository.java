...
public interface OglasiRepository extends CrudRepository<AdvertiseWebNekretnine, Long>{
	@Query(value = "SELECT * FROM test_data5_no_image LIMIT 50 ", nativeQuery = true)
	List<AdvertiseWebNekretnine> getLimitAd();
	
	@Query(value = "SELECT count(*) FROM test_data5_no_image ", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE LOWER(city) LIKE LOWER(?1) AND UPPER(state) LIKE UPPER(?2) AND LOWER(description) LIKE LOWER(%?3%) LIMIT 30", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE  UPPER(full_address) LIKE %:address% AND type_of_property = :type_of_property AND areas BETWEEN :areasmin AND :areasmax AND LOWER(city) LIKE :city AND UPPER(state) LIKE :state AND type_of_ad = :type_of_ad AND ad_published BETWEEN :from AND :to", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE  UPPER(full_address) LIKE %:address% AND type_of_property = :type_of_property  AND LOWER(city) LIKE :city AND UPPER(state) LIKE :state AND type_of_ad = :type_of_ad AND ad_published BETWEEN :from AND :to", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM test_data5_no_image WHERE  UPPER(full_address) LIKE %:address% LIMIT 50", nativeQuery = true)
	...
	@Query(value = "SELECT id_ad, price_per_m, price, areas FROM test_data5_no_image WHERE type_of_property = ?1 AND ad_published BETWEEN ?2 AND ?3", nativeQuery = true)
